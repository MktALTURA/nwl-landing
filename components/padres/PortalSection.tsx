...
export default function PortalSection({ documents, locale }: PortalSectionProps) {
  ...
  if (documents.length === 0) {
    return (
      ...
      >
        <FiInbox className="w-12 h-12 text-n-300 mx-auto mb-4" />
        <h3 className="text-lg font-display font-semibold text-navy/40">
          {t.padres.noDocuments}
        </h3>
        <p className="text-navy/30 mt-1 text-sm">
        ...
        {groups.map((group, gi) => (
          <div key={group.label}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs text-n-500 font-medium">
                {group.docs.length}
              </span>
            </div>
                ...
                >
                  ...
                  <div className="flex gap-2 mt-2.5 pl-11">
                    <a
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-navy border border-navy/20 rounded-md hover:border-gold hover:text-gold-600 transition-colors"
                    >
                    ...
                    <a
                      href={doc.pdfUrl}
                      download
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-[#1C0F00] bg-gold rounded-md hover:bg-gold-400 transition-colors"
                    >
